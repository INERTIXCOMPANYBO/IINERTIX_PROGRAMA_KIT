...
(function () {
  ...
  // Un título líquido
  // ——————————————————————————————————————————————
  function LiquidTitle(el, isStatic) {
    this.el = el;
  ...
  LiquidTitle.prototype.bindEvents = function () {
    ...
    this.el.addEventListener('pointerenter', function (e) {
      ...
      if (self.ready) {
        self.applyForce(self.prevX, self.prevY, CONFIG.impactForceMin);
        wake(self);
      }
    });
    ...
    // Despertar el título al entrar el cursor en la tarjeta
    var card = this.el.closest('.prog-card');
    if (card) {
  ...
  // —— Pool de WebGL para soportar múltiples títulos sin exceder el límite de contextos ——
  function createGLItem() {
    ...
    var uSpecLoc = gl.getUniformLocation(prog, 'uSpec');
    var uAberLoc = gl.getUniformLocation(prog, 'uAber');
  ...
  var GLPool = {
    ...
    acquire: function (owner) {
      for (var i = 0; i < this.items.length; i++) {
        if (!this.items[i].inUse) {
          this.items[i].inUse = true;
          this.items[i].owner = owner;
          return this.items[i];
        }
      }
      if (this.items.length < this.max) {
        var item = createGLItem();
        if (item) {
          item.inUse = true;
          item.owner = owner;
      ...
      var victim = this.items[0];
      for (var j = 1; j < this.items.length; j++) {
        if (this.items[j].owner && victim.owner && this.items[j].owner.amp < victim.owner.amp) {
          victim = this.items[j];
        }
      }
      if (victim.owner) victim.owner.releaseGL();
      victim.inUse = true;
      victim.owner = owner;
      return victim;
    },
  ...
  LiquidTitle.prototype.releaseGL = function () {
    this.ready = false;
    this.el.classList.remove('liquid-text-off');
    running.delete(this);
    if (this.glItem) {
      GLPool.release(this.glItem);
      this.glItem = null;
      this.canvas = null;
      this.gl = null;
      this.prog = null;
  ...
  LiquidTitle.prototype.init = function () {
    ...
    try {
      if (this.isStatic) {
        ...
        var glItem = GLPool.acquire(this);
        if (!glItem) { this.dead = true; return; }
        this.glItem = glItem;
        this.canvas = glItem.canvas;
        this.gl = glItem.gl;
        this.prog = glItem.prog;
        this.buf = glItem.buf;
        this.uTexel = glItem.uTexel;
        this.uAmpLoc = glItem.uAmpLoc;
        this.uSpecLoc = glItem.uSpecLoc;
        this.uAberLoc = glItem.uAberLoc;
        this.texText = glItem.texText;
        this.texWave = glItem.texWave;
      }

      if (!this.measure()) { if (!this.isStatic) this.releaseGL(); return; }
      if (!this.raster()) { if (!this.isStatic) this.releaseGL(); return; }
      if (!this.sanityCheck()) {
        this.dead = true;
        this.el.classList.remove('liquid-text-off');
        if (this.canvas && this.canvas.parentNode) this.canvas.parentNode.removeChild(this.canvas);
        if (!this.isStatic) this.releaseGL();
        return;
      }
      ...
      this.amp = 0;
      this.render();
      this.el.classList.add('liquid-text-off');
    } catch (e) {
      this.dead = true;
      this.el.classList.remove('liquid-text-off');
      if (this.canvas && this.canvas.parentNode) this.canvas.parentNode.removeChild(this.canvas);
  ...
  LiquidTitle.prototype.render = function () {
    ...
    gl.bindBuffer(gl.ARRAY_BUFFER, this.glItem ? this.glItem.buf : this.buf);
  ...
  LiquidTitle.prototype.step = function (dt) {
    ...
    if (this.amp < CONFIG.restAmp) {
      ...
      if (!this.isStatic && !this.pointerOver) {
        this.releaseGL();
      }
  ...
  function scan() {
    ...
    for (var i = 0; i < titles.length; i++) {
      var el = titles[i];
      if (el.__liquid) continue;
      var isStatic = (el.tagName === 'H1' || el.tagName === 'H2');
      var inst = new LiquidTitle(el, isStatic);
      el.__liquid = inst;
      instances.push(inst);
      ...
      if (isStatic) {
        if (io) {
          ...
          (function (t) { idle(function () { t.__liquid.init(); }); })(el);
        }
      }
    }
  }
  ...
  function boot() {
    ...
    if ('IntersectionObserver' in window) {
      io = new IntersectionObserver(function (entries) {
        entries.forEach(function (entry) {
          ...
          idle(function () { if (target.__liquid) target.__liquid.init(); });
        });
      }, { rootMargin: '200px' });
    }
    ...
    window.LiquidText = { instances: instances, refresh: scheduleRefresh, scan: scan };
  }
